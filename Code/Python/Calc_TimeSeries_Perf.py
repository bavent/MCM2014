...
Team = namedtuple('Team', 'Name Year Coach1 Coach2 nRank pRank aRank delR')
Year = namedtuple('Year', 'Y nInv pInv aInv')

dataRoot = 'PythonData_FB/'
#dataRoot = 'PythonData_BB/'

# TODO: Calculate delR and use that to solve for f and C
...
for y in range(1913,2013):
	...
	for line in open(fileName):
		...
		data = line.split(',')
		teams.append(Team(data[0], y, data[1], data[2], int(data[3]), int(data[5]), int(data[7]) , 0))
		if year == None:
	...
	for t in teams:
		...
		normTeams.append(Team(t.Name, t.Year, t.Coach1, t.Coach2, nRank, pRank, aRank, 0))
	teams = normTeams

	# Collect results by coach and team/year
	for t in teams:
